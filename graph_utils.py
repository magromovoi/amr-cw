...
def find_indices(labels, raw_path):
    ...
    # number_match = int(re.search(number_pattern, raw_path).group(0))
...
class ModifyTarget(BaseTransform):
    def __init__(self, val):
        self.val = val

    def __call__(self, data):

        if data.y.numpy()[0] == self.val:
            data.y = torch.tensor(np.asarray([1]), dtype=torch.long)
        else:
            data.y = torch.tensor(np.asarray([0]), dtype=torch.long)
        return data
