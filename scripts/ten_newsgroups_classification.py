...
from bbc_sports_dataset import BBCSportsDataset
...
def get_loaders(dataset, graphs_dataset):

    if dataset == 'ten_newsgroups':

        train_dataset = NewsGroupDataset(root=graphs_dataset)
        test_dataset = NewsGroupDataset(root=graphs_dataset, test=True)

    elif dataset == 'bbcsport':

        train_dataset = BBCSportsDataset(root=graphs_dataset)
        test_dataset = BBCSportsDataset(root=graphs_dataset, test=True)
...
def epoch_iterator(dataset, classes, graphs_dataset, save_model=False):
    train_loader, test_loader = get_loaders(dataset, graphs_dataset)
...
def classify_graphs(dataset, classes, graphs_dataset):
    epoch_iterator(dataset, classes, graphs_dataset)
    # inference(dataset, classes, graphs_dataset)
