...
from amr_cw.graph.classification import classify_graphs
from argparse import ArgumentParser, ArgumentDefaultsHelpFormatter
from amr_cw.concepts.construction import construct_graph_concepts
from amr_cw.graph.concept_whitening import whiten_graph_concepts
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='ten_newsgroups',
                        choices=['ten_newsgroups', 'bbcsport', 'synthetic'],
                        help='dataset to process.')

    parser.add_argument('--operation', type=str, default='graph_concept_whitening',
                        choices=['graph_concept_construction', 'graph_classification',
                                 'graph_concept_whitening', 'evaluation'],
    ...
    if args.dataset == 'ten_newsgroups':
        ...
        dataset = 'synthetic'
        classes = config['synthetic_classes']

        graphs_dataset_prefix = config['synthetic_graphs_dataset_prefix']

        graph_model_name = dataset + graph_conv_type + graph_residual_connections + '_graph_model.pt'
        graph_model_path = input_checkpoints_prefix + '/' + graph_model_name

        concept_pickles_prefix = config['synthetic_concept_pickles_prefix']
        graph_concepts_dataset_prefix = config['synthetic_graph_concepts_dataset_prefix']
        concepts = config['synthetic_graph_concepts']
        concepts_name = '_'.join(concepts)
        concepts_name = '_' + concepts_name
    ...
    if operation == 'graph_concept_construction':
        construct_graph_concepts(concept_pickles_prefix, graph_concepts_dataset_prefix, dataset, classes, concept_type,
                                 ...
                                 config.get('diversity_sim', 'jaccard'))
