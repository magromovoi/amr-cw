...
class iterative_normalization_py(torch.autograd.Function):
    ...
    def forward(ctx, *args, **kwargs):
        ...
        if training:
            ...
            Sigma = torch.baddbmm(input=P[0], batch1=xc, batch2=xc.transpose(1, 2), beta=eps, alpha=1. / m)
            ...
            for k in range(ctx.T):
                ...
                P[k + 1] = torch.baddbmm(input=P[k], batch1=torch.matrix_power(P[k], 3), batch2=Sigma_N, beta=1.5, alpha=-0.5)
            saved.extend(P)
    ...
    def backward(ctx, *grad_outputs):
        ...
        for k in range(ctx.T, 1, -1):
            ...
            g_P.baddbmm_(batch1=P2, batch2=g_tmp, beta=1, alpha=-0.5)
            # g_P.baddbmm_(1, -0.5, P[k - 1].matmul(g_tmp), P[k - 1])
            g_P.baddbmm_(batch1=P[k - 1].matmul(g_tmp), batch2=P[k - 1], beta=1, alpha=-0.5)
        g_sn += g_P
        # g_sn = g_sn * rTr.sqrt()
        ...
        g_x = torch.baddbmm(input=wm.matmul(g_ - g_.mean(-1, keepdim=True)), batch1=g_sigma, batch2=xc)
...
class IterNormRotation(torch.nn.Module):
    ...
    def __init__(self, num_features, num_groups=1, num_channels=None, T=5, dim=4, eps=1e-5, momentum=0.1, affine=False,
                 mode=-1, activation_mode=None, *args, **kwargs):
